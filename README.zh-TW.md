...
本專案使用 Redis 來暫存使用者對話狀態，以確保流暢的使用者體驗。

### 1. 寫入時機 (Set)
當使用者在對話中進行選擇時，系統會將狀態暫存：
- **選擇主題時**：記錄使用者選擇的「主題 (Theme)」。
- **選擇風格時**：記錄使用者選擇的「風格 (Style)」。

系統會使用對應的 `sourceId` (User ID) 作為鍵值 (Key) 來儲存 `UserState` 物件。

### 2. 刪除時機 (Delete)
為了允許重新開始並避免狀態混亂，Redis 中的資料會在以下情況被刪除：
- **流程結束**：當圖片生成完畢並成功傳送給使用者後，系統會自動清除狀態。
- **手動重置**：當使用者輸入「開始」、「生成圖片」等觸發關鍵字欲重新開始流程時，舊的狀態會被清除。
- **自動過期**：若使用者在選擇後超過 **5 分鐘** (預設 `300` 秒) 未進行下一步，Redis 的 TTL (Time To Live) 機制會自動刪除該筆資料。

## 🔒 隱私與資料安全 (Cloudinary 圖片儲存策略)

為了保障您的隱私並節省資源，本服務採用「即刻生成、即刻刪除」的圖片處理策略：

1.  **寫入時機**：當 AI 生成圖片並完成文字疊加後，系統會立即將圖片上傳至 Cloudinary 以取得公開 URL。
2.  **刪除時機**：一旦圖片 URL 成功傳送至 LINE 給您，系統會等待約 10 秒（緩衝時間），隨即呼叫刪除指令將圖片從 Cloudinary 移除。
3.  **刪除原因**：
    - **LINE 轉傳機制**：圖片透過 LINE 傳送後，會暫存在 LINE 的伺服器上，您已可直接轉傳或分享，無須依賴 Cloudinary 的連結。
    - **成本與隱私考量**：此舉能大幅降低 Cloudinary 的儲存成本，並確保您的客製化圖片不會被永久保留在第三方伺服器上。
